...
public:
    ...
    bool sort_main(const char *input_fname);

private:
    bool srt_stage1(const char *input_fname);
    bool srt_stage2_merge_files();
    ...
    static const int merge_files_max_k_                    = 16; // По сколько файлов объединять за раз. Можно увеличить.
    ...
    int stage_i_;              // Номер "поколения" созданных временных файлов. == 1 после многопоточной сортировки.
    int stage_files_;          // Сколько временных файлов было создано.
